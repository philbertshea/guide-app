...
export default function WhatsappPage() {
  const bind = useLongPress(() => {
    navigate('/whatsapp/delete_msg/3');
  });
  ...
  return (
    <main className="flex flex-col items-center justify-center">
      ...
      <div className={`relative overflow-hidden flex items-center justify-center flex-col w-[calc(100vw-(100vh-60px)/${HEIGHT_TO_WIDTH})] h-[calc(100vh-60px)]`}>
        <Image 
        className="img-bg"
        alt="whatsapp_deletemsg_2"
        src="/whatsapp/wa_deletemsg_2.jpeg"
        width={0}
        height={0}
        sizes="100vh"
        style={{ height: '100%', width: 'auto'}}
        />     
        <div className="absolute w-[calc((100vh-60px)/2.223*0.62)] h-[calc((100vh-60px)*0.06)] bottom-[calc((100vh-60px)*0.11)] right-0 flex">
        ...
        <div className="absolute w-[calc((100vh-60px)/2.223*0.22)] h-[calc((100vh-60px)*0.06)] bottom-[calc((100vh-60px)*0)] right-[calc((100vh-60px)/2.223*0.18)] flex">
          <Link href="/whatsapp/delete_msg/1" className="text-xl w-full rounded border-4 border-red-800 bg-red-100 opacity-30"></Link>
        </div>
